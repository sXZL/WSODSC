...
class ROIWeakHead(torch.nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        self.CONTEXT_ON = cfg.CTX.CTX_ON
    ...
    def forward(self, features, proposals, targets=None, model_cdb=None):
        ...
        if len(proposals[0]) == 2:
            proposal_boxes = [i[0] for i in proposals]
            context_boxes = [i[1] for i in proposals]
        else:
            proposal_boxes = proposals
    
        if self.CONTEXT_ON == False:
            roi_feats  = self.feature_extractor(features, proposal_boxes)
            cls_score, det_score, ref_scores = self.predictor(roi_feats, proposal_boxes)
        else:
            roi_feats  = self.feature_extractor(features, proposal_boxes)
            ctx_feats  = self.feature_extractor(features, context_boxes)
            cls_score, det_score, ref_scores = self.predictor(roi_feats, ctx_feats, proposal_boxes)
        ...
        if not self.training:
            ...
            result = self.post_processor(final_score, proposal_boxes)
            return roi_feats, result, {}, {}

        loss_img, accuracy_img = self.loss_evaluator([cls_score], [det_score], ref_scores, proposal_boxes, targets)
...
class ROIWeakRegHead(torch.nn.Module):
    ...
    def forward(self, features, proposals, targets=None, model_cdb=None):
        ...
        if self.roi_sampler is not None and self.training:
            with torch.no_grad():
                proposal_boxes = self.roi_sampler(proposal_boxes, targets)

        if self.CONTEXT_ON == False:
            roi_feats  = self.go_through_cdb(features, proposal_boxes, model_cdb)
            cls_score, det_score, ref_scores, ref_bbox_preds = self.predictor(roi_feats, proposal_boxes)
        else:
            roi_feats  = self.go_through_cdb(features, proposal_boxes, model_cdb)
            ctx_feats  = self.go_through_cdb(features, context_boxes, model_cdb)
            cls_score, det_score, ref_scores, ref_bbox_preds = self.predictor(roi_feats, ctx_feats, proposal_boxes)

        if not self.training:
            result = self.testing_forward(cls_score, det_score, proposal_boxes, ref_scores, ref_bbox_preds)
            return roi_feats, result, {}, {}
        loss_img, accuracy_img = self.loss_evaluator([cls_score], [det_score], ref_scores, ref_bbox_preds, proposal_boxes, targets)
        return (roi_feats, proposal_boxes, loss_img, accuracy_img)
